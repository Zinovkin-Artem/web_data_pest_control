...
def diagramma(_pred, bar, z_po, flag = True): 
    if flag:   
        ...
        if bar == "I":
            ...
            bars2 = ax.bar(x, values2, width=width, label="Другий бар'єр", color='orange', alpha=0.7)
            ax.bar_label(bars2, padding=3, fontsize=10, color='black')

            if toggle_state_1:
                bars1 = ax.bar(x - width, values1, width=width, label="Перший бар'єр", color='blue', alpha=0.7)
                ax.bar_label(bars1, padding=3, fontsize=10, color='black')
